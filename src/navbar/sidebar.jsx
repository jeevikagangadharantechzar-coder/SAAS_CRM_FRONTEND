...
const FacebookIcon = ({ size = 18, color = "#475569" }) => (
  <svg viewBox="0 0 24 24" fill={color} width={size} height={size}>
    <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
  </svg>
);

const InstagramIcon = ({ size = 18, color = "#475569" }) => (
  <svg viewBox="0 0 24 24" fill={color} width={size} height={size}>
...
const Sidebar = ({ isOpen, toggleSidebar }) => {
  ...
  useEffect(() => {
    ...
    if (p.includes("/whatsapp") || p.includes("/instagram") || p.includes("/facebook")) {
      setShowMeta(true);
    }
  }, [p, isAnyTaskActive]);
  ...
  return (
    ...
    >
      ...
      <nav className={`flex flex-col gap-2.5 ${isOpen ? "px-1.5" : "px-0.5 items-center"}`}>
        ...
        {/* Meta Integrations (WhatsApp, Instagram, Facebook) — Admin only */}
        {isAdmin && (hasPlanFeature("integration_whatsapp") || hasPlanFeature("integration_instagram") || hasPlanFeature("integration_facebook")) && (
          <Collapsible
            ...
            sidebarOpen={isOpen}
            activePaths={["/whatsapp", "/instagram", "/facebook"]}
          >
            {hasPlanFeature("integration_whatsapp") && (
              <SmallLink
                to="whatsapp"
                icon={<WhatsAppIcon />}
                label="WhatsApp"
            ...
            {hasPlanFeature("integration_instagram") && (
              <SmallLink
                to="instagram"
                icon={<InstagramIcon />}
                label="Instagram"
            ...
            {hasPlanFeature("integration_facebook") && (
              <SmallLink
                to="facebook"
                icon={<FacebookIcon />}
                label="Facebook"
