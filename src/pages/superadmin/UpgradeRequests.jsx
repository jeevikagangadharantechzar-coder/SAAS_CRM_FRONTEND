  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <h2 className="text-slate-900 dark:text-white">Upgrade Requests</h2>
          <p className="text-base text-slate-600 dark:text-slate-400">Review, verify pricing, and manage plan upgrade requests submitted by tenant admins.</p>
        </div>
      ...
      {activeTab === "pending" ? (
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 shadow-sm rounded-2xl overflow-hidden">
          <div className="px-6 py-4 bg-slate-50 dark:bg-slate-800/80 border-b border-slate-100 dark:border-slate-800 flex items-center space-x-2">
            <ArrowUpCircle className="text-[#008ecc] dark:text-[#33b8ff]" size={20} />
            <h3 className="text-slate-700 dark:text-slate-300">Pending Upgrade Requests</h3>
          </div>
                      ...
                      <tr key={req._id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                        <td className="px-6 py-4">
                          <div className="flex flex-col">
                            <span className="font-bold text-slate-900 dark:text-white">{req.tenant_id?.name || req.tenant_name || "N/A"}</span>
                            <span className="text-xs text-slate-500 dark:text-slate-400 font-mono">Slug: {req.tenant_id?.slug || req.tenant_slug || "N/A"}</span>
                          </div>
                        </td>
                        <td className="px-6 py-4">
                          <div className="font-bold text-slate-900 dark:text-white uppercase">{req.plan_id?.plan_name || req.plan_name || "N/A"}</div>
                          {req.billing_cycle && (
                            <div className="text-xs text-[#008ecc] dark:text-[#33b8ff] font-semibold mt-0.5">
                              {{ monthly: "Monthly", half_yearly: "Half Year", yearly: "Yearly" }[req.billing_cycle] || req.billing_cycle}
                            </div>
                          )}
                          {req.description && (
                            <div className="text-xs text-slate-400 dark:text-slate-500 mt-1 max-w-[200px] truncate" title={req.description}>
                              "{req.description}"
                            </div>
                          )}
                        </td>
                        ...
                        <td className="px-6 py-4 font-medium">
                          <span className={`inline-flex items-center px-2.5 py-0.5 rounded text-xs font-bold border uppercase ${
                            req.type === "mid_cycle"
                              ? "bg-blue-50 dark:bg-blue-900/30 text-blue-700 border-blue-100 dark:border-blue-800/50"
                          ...
                          {(() => {
                            ...
                            return (
                              <div className="space-y-1">
                                <div className="text-xs text-slate-400 dark:text-slate-500 font-semibold uppercase tracking-wider">Plan Price</div>
                                <div className="font-extrabold text-slate-800 dark:text-slate-200 text-base">${planPrice.toLocaleString()}</div>
                                {req.prorated_discount > 0 && (
                                  <div className="text-xs text-emerald-600 dark:text-emerald-400 font-semibold">− ${req.prorated_discount.toFixed(2)} credit</div>
                                )}
                                <div className={`text-xs font-bold ${toCharge === 0 ? "text-emerald-600 dark:text-emerald-400" : "text-[#008ecc] dark:text-[#33b8ff]"}`}>
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 shadow-sm rounded-2xl overflow-hidden">
          <div className="px-6 py-4 bg-slate-50 dark:bg-slate-800/80 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
            <div className="flex items-center space-x-2">
              <History className="text-[#008ecc] dark:text-[#33b8ff]" size={20} />
              <h3 className="text-slate-700 dark:text-slate-300">Processed Upgrade History</h3>
            </div>
                  ...
                  paginatedHistoryRequests.map((req) => {
                    ...
                    return (
                      <tr key={req._id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                        ...
                        <td className="px-6 py-4 font-extrabold text-[#008ecc] dark:text-[#33b8ff] text-base">
                        ...
                        <td className="px-6 py-4 text-right">
                          <div className="flex items-center justify-end gap-3">
                            ...
                            <button
                              ...
                              disabled={deletingHistory}
                              title="Delete record"
                              className="text-slate-400 dark:text-slate-500 hover:text-red-600 transition-colors disabled:opacity-40 cursor-pointer"
                            >
                              <Trash2 size={16} />
                            </button>
                          </div>
      ...
      {isRejectModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-2xl w-full max-w-md overflow-hidden transform transition-all animate-in fade-in zoom-in-95 duration-250">
            {/* Modal Header */}
            <div className="px-6 py-4 bg-slate-50 dark:bg-slate-800/80 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
              <div className="flex items-center space-x-2 text-red-600">
                <XCircle size={20} />
                <h3 className="text-slate-700 dark:text-slate-300">Reject Upgrade Request</h3>
              </div>
              <button
            ...
            <form onSubmit={handleRejectSubmit} className="p-6 space-y-4">
              {rejectingRequestData && (
                <div className="bg-slate-50 dark:bg-slate-800/80 rounded-lg p-3 text-xs text-slate-600 dark:text-slate-400 border border-slate-100 dark:border-slate-800 space-y-1">
                  <div>
              ...
              <div className="space-y-1">
                ...
                <textarea
                  ...
                  className="w-full px-3 py-2 text-sm bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 rounded-lg focus:outline-none focus:border-red-400 focus:bg-white dark:bg-slate-900 transition-colors resize-none text-slate-800 dark:text-slate-200"
                ></textarea>
              </div>
              ...
              <div className="flex items-center justify-end space-x-3 pt-2">
                <button
                  type="button"
                  onClick={() => setIsRejectModalOpen(false)}
                  className="px-4 py-2 bg-slate-100 dark:bg-slate-800/50 hover:bg-slate-200 text-slate-700 dark:text-slate-300 font-bold text-xs rounded-lg transition cursor-pointer"
                >
                  Cancel
                </button>
                <button
