...
const DealsDocument = () => {
  ...
  useEffect(() => {
    ...
    const fetchDeals = async () => {
      ...
      try {
        ...
        if (active) toast.error("Failed to fetch deals");
      } finally {
  ...
  const filteredDeals = useMemo(
    () => deals.filter((d) => d.dealName?.toLowerCase().includes(searchQuery.trim().toLowerCase())),
    [deals, searchQuery]
  );
  ...
  return (
    <div className="p-4">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-4 gap-3">
        <h2 className="text-xl font-semibold text-gray-800">Deal Documents</h2>
      </div>
      ...
      <div className="overflow-x-auto rounded-lg border border-gray-200 shadow-sm">
          ...
          <tbody className="divide-y divide-gray-200">
            {paginatedDeals.length > 0 ? (
              paginatedDeals.map((deal, idx) => (
                <tr key={deal._id} className={`${idx % 2 === 0 ? "bg-white" : "bg-gray-50"} hover:bg-gray-50`}>
        ...
        <div className="flex items-center justify-end gap-6 border-t border-gray-200 bg-white px-4 py-2 text-sm text-gray-600">
          ...
          <span>
            {firstItem}–{lastItem} of {totalDeals}
          </span>
      ...
      <Dialog open={!!attachmentsDeal} onOpenChange={(open) => !open && setAttachmentsDeal(null)}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle className="text-gray-800">
              Attachment — {attachmentsDeal?.dealName}
            </DialogTitle>
          </DialogHeader>

          {attachmentsDeal?.attachments?.length > 0 ? (
            <ul className="space-y-3">
              {attachmentsDeal.attachments.map((file, idx) => {
